...
class SPARTA:
    def __init__(self, model_path: str = None, sep: str = " ", sparse_vector_dim: int = 2000, max_length: int = 500,
                 **kwargs):
        self.sep = sep
    ...
    def encode_corpus(self, corpus: List[Dict[str, str]], batch_size: int = 16, **kwargs):
        ...
        col = np.zeros(num_elements, dtype=np.int32)
        row = np.zeros(num_elements, dtype=np.int32)
        ...
        return csr_matrix((values, (row, col)), shape=(len(self.bert_input_embeddings), len(sentences)),
                          dtype=np.float32)
